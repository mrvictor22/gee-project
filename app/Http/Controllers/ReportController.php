...
class ReportController extends Controller
{
    ...
    public function dailySale($year, $month)
    {
        ...
        if($role->hasPermissionTo('daily-sale')){
            ...
            while($start <= $number_of_day)
            {
                ...
                $aprobados = Ticket_cors::whereDate('created_at', $date)->where('estado_ticket','Anulado')->count();
            ...
            $lims_warehouse_list = Warehouse::where('is_active', true)->get();
            $warehouse_id = 0;
            return view('report.daily_sale', compact('taprobado','total_discount','order_discount', 'total_tax', 'order_tax', 'shipping_cost', 'grand_total', 'start_day', 'year', 'month', 'number_of_day', 'prev_year', 'prev_month', 'next_year', 'next_month', 'lims_warehouse_list', 'warehouse_id'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
    ...
    public function customerReport(Request $request)
    {
        ...
        return view('report.customer_report', compact('customer_name','lims_sale_data','customer_id', 'start_date', 'end_date', 'lims_product_sale_data', 'lims_payment_data', 'lims_customer_list', 'lims_quotation_data', 'lims_product_quotation_data', 'lims_return_data', 'lims_product_return_data'));
    }
    ...
    public function productprodCost(Request $request)
    {        
        ...
        if($warehouse_id == 0) {                   
            ...
            $lims_product_all = Procesosl::whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->get();
            }
        else
            {
            $lims_product_all = Procesosl::where('producto_id', $warehouse_id)->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->get();
          }
        foreach ($lims_product_all as $product) {
                    ...
                    $product_id[] = $product->id;
                    $bolsas_utilizar[] = $product->bolsas_utilizar;
                    $costo_enbolsado[] = $product->costo_enbolsado;                   
                    $etiquetas_utilizar[] = $product->etiquetas_utilizar;
                    $costo_etiquetas[] = $product->costo_etiquetas;
                    $costo[] = $product->costo_etiquetas + $product->costo_enbolsado;
                    $variant_id[] = null;
                    $product_qty[] = null;
                   /* if($warehouse_id == 0)
                        $product_qty[] = $product->qty;
        ...
        $lims_warehouse_list = Product::where([['is_active', true],['type', 'standard'],['category_id', '1']])->get();
        //Product::where([ ['category_id', '1'], ['type', 'standard'] ])->get(); 
        return view('report.product_prod_proc',compact('product_id', 'variant_id', 'product_name', 'product_qty', 'start_date', 'end_date', 'lims_warehouse_list', 'warehouse_id','bolsas_utilizar','costo_enbolsado','etiquetas_utilizar','costo_etiquetas','costo'));
    }
}
