...
class SaleController extends Controller
{
    ...
    public function saleData(Request $request)
    {
        ...
        if(!empty($sales))
        {
            foreach ($sales as $key=>$sale)
            {
                ...
                $nestedData['key'] = $key;
                $nestedData['date_r'] = $sale->fecha_realizada;
                ...
                elseif($sale->sale_status == 2){
                    ...
                    $sale_status = trans('file.Draft');
                        ...
                        //AQUI MUESTRA EL ESTADO DE PAGO EN EL INDEX DEL MODULO DE SALES
                ...
                elseif($sale->payment_status == 3)
                   ...
                   $datal =  $sale->estado_ticket;
                    //AQUI SE DEJA EN INDEX EL ESTADOD EL TICKET
                if($sale->estado_ticket == 'Anulado')
                    $nestedData['estado_ticket'] = '<div class="badge badge-danger">'.$datal .'</div>';
                else
                    $nestedData['estado_ticket'] = '<div class="badge badge-success">'.$datal.'</div>';
                ...
                $nestedData['options'] = '<div class="btn-group">
                            ...
                            <ul class="dropdown-menu edit-options dropdown-menu-right dropdown-default" user="menu">
                                <li><a href="'.route('sale.invoice', ['id' => $sale->id]).'" class="btn btn-link"><i class="fa fa-copy"></i> '.trans('Generar Ticket').'</a></li>
                                <!--Generacion de los demas tipos de documentos de tickets y facturas-->
                               
                                <li><a href="'.route('sale.invoicefactura', ['id' => $sale->id]).'" class="btn btn-link"><i class="fa fa-copy"></i> '.trans('Generar Factura Consumidor final').'</a></li>
                                <li><a href="'.route('sale.invoicecf', ['id' => $sale->id]).'" class="btn btn-link"><i class="fa fa-copy"></i> '.trans('Generar Credito Fiscal').'</a></li>
                               
                                <li>
                                    <button type="button" class="btn btn-link view"><i class="fa fa-eye"></i> '.trans('file.View').'</button>
                                </li>';
                                /**Generacion de los demas tickets arriba! */
    ...
    public function create()
    {
        ...
        if($role->hasPermissionTo('sales-add')){
            ...
            $lims_pos_setting_data = PosSetting::find(1);
    ...
    public function store(Request $request)
    {
        ...
        if($data['payment_status'] == 3 || $data['payment_status'] == 4 || ($data['payment_status'] == 2 && $data['pos'] && $data['paid_amount'] > 0)) {
            ...
            //AQUI SE MODIFICARIA PARA SOLOA ALMACENAR LOS DATOS DEL MONTO DE PAGO POR TARJETA, QUITAR TODO LO REFERIDO A STRIPE
        ...
        //AQUI SE DECIDE QUE TIPO DE DOCUMENTO SE IMPRIME AL MOMENTO DE UNA VENTA Y SI ES PROVINIENTE DE LA PANTALLA POS
        //PARA EL CASO DE PROYECTO LECHERIA EN EL SUBDOMINIO DE POS CAMBIAR EL DOCUMENTO A IMPRIMIR POR gen_invoiceticketh
        //EN LA OPCION 1 QUE ES LA QUE ESTA A CONTINUACION
        if($lims_sale_data->sale_status == '1' && $data['pos']&& $data['doc_f'] == '1' )
            return redirect('sales/gen_invoiceticketh/' . $lims_sale_data->id)->with('message', $message);
            elseif($lims_sale_data->sale_status == '1' && $data['pos']&& $data['doc_f'] == '2' )
            return redirect('sales/gen_invoicefactura/' . $lims_sale_data->id)->with('message', $message);
            elseif($lims_sale_data->sale_status == '1' && $data['pos']&& $data['doc_f'] == '3' )
            return redirect('sales/gen_invoicecf/' . $lims_sale_data->id)->with('message', $message);
        elseif($data['pos'])
   ...
   // }
    ...
    public function edit($id)
    {
        ...
        if($role->hasPermissionTo('sales-edit')){
           ...
           $lims_venta_corr = Ticket_cors::where('venta_id',$id)->get();
  
           //AQUI OBTENGO EL REGISTRO DEl ESTADO DEL TICKET ALOJADO EN LA TABLA TICKETSCORS
               
             $idventa=0;
           foreach($lims_venta_corr as $cor){
              $idventa= $cor->estado_ticket;
                   }
           
            return view('sale.edit',compact('idventa','lims_customer_list', 'lims_warehouse_list', 'lims_biller_list', 'lims_tax_list', 'lims_sale_data','lims_product_sale_data'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        //return dd($data);
        $document = $request->document;
        $estado=$data['estado_tick'];

        $lims_venta_corr = Ticket_cors::where('venta_id',$id)->update(['estado_ticket' => $estado]);     
        $lims_venta_corr = Sale::where('id',$id)->update(['estado_ticket' => $estado]);     
        ...
        foreach ($lims_product_sale_data as  $key => $product_sale_data) {
            ...
            elseif( ($lims_sale_data->sale_status == 1) && ($product_sale_data->sale_unit_id != 0)) {
                //AQUI ES 1 CUANDO ESTA TERMINADO LA VENTA
    ...
    public function genInvoice($id)
    {
        ...
        if(\App::getLocale() == 'ar' || \App::getLocale() == 'hi' || \App::getLocale() == 'vi' || \App::getLocale() == 'en-gb')
            ...
            $convertir=$lims_sale_data->grand_total;
            $int = (int)$convertir;
            $fraction = ($convertir*100)%100;
          $numberInWords = $numberTransformer->toWords($int);
          $numberInWords2 = $numberTransformer->toWords($fraction);

        return view('sale.invoice', compact('lims_sale_data', 'lims_product_sale_data', 'lims_biller_data', 'lims_warehouse_data', 'lims_customer_data', 'lims_payment_data', 'numberInWords','numberInWords2','fraction'));
    }
    public function genInvoicecf($id)
    {
        $lims_sale_data = Sale::find($id);
        $lims_product_sale_data = Product_Sale::where('sale_id', $id)->get();
        $lims_biller_data = Biller::find($lims_sale_data->biller_id);
        ...
        foreach ($lims_product_sale_data as $key => $producto_sale) {
            $sacandoiva=number_format((float)($producto_sale->net_unit_price ), 3, '.', '') /1.13;
            $ventasafectas2 = number_format((float)($sacandoiva *$producto_sale->qty ), 2, '.', '');
        ...
        return view('sale.invoicecf', compact('lims_sale_data', 'lims_product_sale_data', 'lims_biller_data', 'lims_warehouse_data', 'lims_customer_data', 'lims_payment_data', 'numberInWords','numberInWords2','fraction'));
    }
    public function genInvoicetickh($id)
    {
        $lims_sale_data = Sale::find($id);
        $lims_product_sale_data = Product_Sale::where('sale_id', $id)->get();
        ...
        $lims_customer_data = Customer::find($lims_sale_data->customer_id);
        $lims_payment_data = Payment::where('sale_id', $id)->get();
             //CON lims_product_sale_data SACAR LA CANTIDAD DE PRODUCTOS VENDIDOS EN 1 TICKECT PARA GUARDARLO EN EL LOG
             $cantidad_prod=Product_Sale::where('sale_id', $id)->count('product_id');
        $lims_venta_corr = Ticket_cors::where('venta_id',$id)->get();
  
      
        //AQUI OBTENGO EL REGISTRO DE LA VENTA ALOJADO EN LA TABLA TICKETSCORS
            
          $idventa=0;
        foreach($lims_venta_corr as $cor){
           $idventa= $cor->venta_id;
                }

                if ($idventa == $id) {
                    //JALAR DEL LA TABLA LOG EL ID DEL TICKET QUE SE GENERO EN TODO CASO
                    $corr_siguiente=$cor->correlativo_no;
                   
                    
                }else {
                    //guardo en la tabla de log de los tickets, instancio la creacion para esta tabla
                        $ticket_save = new Ticket_cors;
                    //guardo los datos
                    $ticket_save->venta_id=$id;
                    $ticket_save->caja_id=$lims_sale_data->biller_id;
                    $ticket_save->vendedor_id=$lims_sale_data->user_id;
                     //TOMO EL CORRELATIVO ACTUAL Y LO GUARDO
                     $corr_actual=$lims_biller_data->correlativo_actual;
                     //DEL CORRELATIVO ACTUAL SUMO + 1
                     $corr_siguiente= $corr_actual+1;

                     $lims_biller_data->correlativo_actual =$corr_siguiente;

                    $ticket_save->correlativo_no=$corr_siguiente;
                    $ticket_save->cantidad_productos=$cantidad_prod;
                    $ticket_save->fecha_venta=$lims_sale_data->created_at;
                    $ticket_save->total_venta=$lims_sale_data->grand_total;
        ...
        return view('sale.invoiceticketh', compact('corr_siguiente','lims_sale_data', 'lims_product_sale_data', 'lims_biller_data', 'lims_warehouse_data', 'lims_customer_data', 'lims_payment_data', 'numberInWords','numberInWords2','fraction'));
    }
    public function genInvoicefactura($id)
    {
        $lims_sale_data = Sale::find($id);
        $lims_product_sale_data = Product_Sale::where('sale_id', $id)->get();
        $lims_biller_data = Biller::find($lims_sale_data->biller_id);
        ...
        return view('sale.invoicefactura', compact('lims_sale_data', 'lims_product_sale_data', 'lims_biller_data', 'lims_warehouse_data', 'lims_customer_data', 'lims_payment_data', 'numberInWords','numberInWords2','fraction'));
    }
    ...
    public function deleteBySelection(Request $request)
    {
        ...
        foreach ($sale_id as $id) {
            $lims_sale_data = Sale::find($id);
            $lims_sale_datat = Ticket_cors::where('venta_id',$id);
