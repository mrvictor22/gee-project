...
class ProductController extends Controller
{
    ...
    public function productData(Request $request)
    {
        $columns = array( 
            ...
            7 => 'unit_id',
            8 => 'price' ,
            9 =>'mano_obra',//indice de la columna mano de obra
            10 =>'tiempo_produccion'
        ...
        if(!empty($products))
        {
            foreach ($products as $key=>$product)
            {
                ...
                $nestedData['mano_obra'] = $product->mano_obra;//leyendo dato mano de obra
                $nestedData['producto_derivado'] = $product->producto_derivado;//leyendo dato mano de obra
                $nestedData['tiempo_produccion'] = $product->tiempo_produccion;
                if($product->brand_id)
    ...
    public function create()
    {
        ...
        if ($role->hasPermissionTo('products-add')){
            ...
            $lims_product_list2 = Product::where([ ['is_active', true], ['type', 'standard'],['category_id','2'] ])->get();
            $lims_product_lista = Product::where('is_active', true)->get();
            $lims_brand_list = Brand::where('is_active', true)->get();
            $lims_category_list = Category::where('is_active', true)->get();
            $lims_unit_list = Unit::where('is_active', true)->get();
            $lims_tax_list = Tax::where('is_active', true)->get();
            return view('product.create',compact('lims_product_list2','lims_product_list','lims_product_lista', 'lims_brand_list', 'lims_category_list', 'lims_unit_list', 'lims_tax_list'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
